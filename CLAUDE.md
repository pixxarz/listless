...
| `cleanText()` | 🚨 **Fixes the root cause, so the report side stops having to mop up.** Every text field in `collectData()` goes through it before the payload is built: NFC, strip zero-width/BOM/soft-hyphen, fold exotic spaces, collapse whitespace runs, trim. It only ever removes characters that **change nothing visually** — it must never touch the teacher's spelling, and it deliberately does *not* reorder Thai marks (that stays a comparison-only transform in `visKey`). Before this, a teacher hitting the space bar twice created a second copy of a student in every card, chart, table and printed document |
...
- **Instant second login (`CACHE_KEY = 'chanu_report_cache_v1'`).** `doGet` reads *two* sheets per call (`readSheet()` + `readFixes()`) and takes 10–29 s; the backend is the friend's Apps Script and **cannot be redeployed from this repo**, so the only lever is the browser. On success the response is cached; on the next login the dashboard paints from cache immediately while the real request runs, then swaps. Guards, all of which exist for a reason: the cache is keyed by a **SHA-256 of the password** (never the password itself) so a wrong password shows nothing and wipes it; it expires after `CACHE_TTL` (12 h); "ออกจากระบบ" clears it, since closing the browser does not. If `crypto.subtle` is unavailable, caching is skipped entirely rather than falling back to a weak key. A `.stale-bar` is mandatory whenever cached data is on screen — unlabelled stale data is worse than a slow page. When the network leg fails while the cache is showing, **do not bounce back to the gate**; keep the data up with the amber bar (this is the only thing that works during the Apps Script 404 outages). `initApp()` is guarded by `appReady` because it binds listeners and now runs twice per login.
...
- 🚨 **"เชื่อมต่อไม่สำเร็จ" on a user's machine is usually their Google session, not the network.** Apps Script returns **404** when the browser has **multiple Google accounts signed in** — even with the deployment set to "Anyone", even for the script's own owner. Proof from a real device: same machine, same network, same minute — normal window `302 → 404`, InPrivate window `302 → 200`. Fix is on the user side (sign out of all but one Google account, or use a private window); it cannot be fixed in this code. Requests also legitimately take **10–29 s** sometimes, which is why the per-attempt timeout is 45 s.
...
- 🚨 **Counting people: there is exactly one identity rule, and it lives in `visKey()` in `js/fixes.js`.** `report.js` delegates to it via `FIXES.visKey` — **never write a second copy**; if the two files normalise differently, the KPI card and the charts silently disagree and nobody can tell which is right. It strips zero-width/BOM/soft-hyphen, folds exotic spaces, collapses runs of whitespace, and **sorts the Thai vowel/tone marks attached to each consonant**. That last one matters: `กี่` and `ก่ี` render identically but are different strings, and `normalize('NFC')` does **not** fix it (verified — Unicode has no canonical order for Thai marks). It is a **comparison key only**; never display it, never write it to the sheet.
- 🚨 **Charts count people, not rows.** `renderChart`/`renderSubjectChart`/`renderPercentChart` all go through `countStu()` (which is `visKey` on the name). Counting raw rows made ม.1–ม.6 sum to 737 while the card said 148, because one student reported in 5 subjects is 5 rows. The tooltip shows both numbers (`41 คน (203 รายการ)`) so the row count is not lost. When the bars legitimately exceed the card — one name appearing under two grade levels — `renderChart` appends a `.chart-note` explaining why, rather than letting two numbers contradict each other in silence.
- 🚨 **`buildStudentGroups` keys on `visKey(name)+'|'+visKey(cls)`.** It used to concatenate the **raw** `r[COL.cls]`, so `ม.2/1` and `ม.2/1 ` (one trailing space) split a student into two identical-looking rows that were impossible to tell apart on screen. When a merge hides a real disagreement, the group keeps every variant (`seats`, `prefixes`, `names`, `clsList`) and `dispVar()` shows them all — **but only when they differ visibly**. Joining two variants that look the same produced `ธนธรณ์ มณีเลิศ / ธนธรณ์ มณีเลิศ`, which reads as a broken page. Invisible differences are explained by the red chip instead.
- 🚨 **The student popup counts "รายชื่อ", not "คน".** One person keyed into two rooms is 2 รายชื่อ but 1 คน; labelling both "คน" made the card (148) contradict the popup it opens. The count line under the filter bar spells out the gap. `findNameDups()` drives the highlighting and follows one rule: **a red cell border only ever goes on a cell that already displays two values** (`36, 37`). Bordering a cell whose contents look perfectly normal tells the reader nothing — that case gets a plain-Thai chip (`ชื่อในชีตไม่ตรงกัน (ตาดูเหมือนกัน)`). Every flagged row carries a chip; hover tooltips are a supplement, never the only explanation.
- 🚨 **The printed document shows one value per field, always.** `buildStudentListDoc` deliberately does **not** use `dispVar` — printing `เลขที่ 22, 23` on a government form reads as a broken document. Conflicts belong on screen, where they get fixed. The merge fix still applies, so one student is one numbered entry instead of two.
...
- **Data fixes (`js/fixes.js`)** — teachers type the same person/room/subject differently, so counts split. The engine proposes, a human always decides, and decisions overlay the data at display time (`FIXES.apply()` runs inside `normalizeRows`, so cards/charts/table/CSV all move together). `restore()` runs first on every apply, making it idempotent and making undo instant.
  ...
  - 🚨 **`detectInvisible()` exists because every other detector normalises the difference away before it can see it.** `rawName()` collapses whitespace runs and `phonetic()` strips spaces entirely, so `ธนธรณ์ มณีเลิศ` vs `ธนธรณ์  มณีเลิศ` (two spaces — a real case, confirmed from the sheet's character codes) compare equal, produce no pair, and vanish. It groups by `visKey` and compares the **raw** strings inside each group. Rooms are not duplicated here: `detectRooms` already groups on raw `cls`. Each option carries `describeOdd()` output — `เคาะเว้นวรรค 2 ที ติดกัน`, `มีอักขระที่มองไม่เห็นปนอยู่ 1 ตัว` — because `diffHTML()` cannot highlight something with no visible glyph, and a card showing two identical-looking options with no explanation just makes people guess.
  - **The surname guard is `lev(...)>2`, not `!==`.** Requiring phonetically identical surnames meant a typo *in the surname* (`ธิวาธร ถานิช`/`ถานิชย์`, `วรัขยา โมยา`/`โมหา`) was never once proposed in the panel's lifetime. Siblings are still blocked by the grade-level guard above it and the first-name guard below it — do not remove either.
...
- `js/report.js`: `CACHE_KEY = 'chanu_report_cache_v1'`, `CACHE_TTL = 12 h`, `CACHE_MAX = 4 MB` (skip caching rather than blow the localStorage quota)
- `js/fixes.js`: `TH_MARK` (the Thai vowel/tone marks `visKey` sorts) — comparison only, never displayed or written
