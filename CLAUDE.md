...
- 🚨 **Every call retries — do not remove it.** `jsonp()` wraps `once()` and retries up to 3 times (1s then 2s backoff) on `network`/`timeout` only; auth failures fire once. Timeout per attempt is 45s (the sheet grew and `doGet` reads two sheets per call; 20s was no longer enough). Real-world failure: `ERR_NAME_NOT_RESOLVED` then `ERR_NETWORK_CHANGED` on the first two attempts, the third succeeding — Edge and Safari resolve DNS through their own resolver, so they stumble where Chrome does not. Without retry that reads as "permanently broken on Edge/Safari, fine on Chrome", which is exactly the bug this fixed. On timeout the callback is **replaced with a no-op instead of deleted**, so a late response is swallowed quietly rather than throwing `callback is not defined`. `js/fixes.js` has its own `jsonpGet()` with the same retry — **any new transport added anywhere must carry it too**.
- 🚨 **All backend calls go through `/gs`, never `script.google.com` directly.** `_redirects` proxies `/gs` to the Apps Script URL with **status 200** — a 301/302 would send the browser to Google itself and reproduce the bug. Reason: Edge and Safari resolve DNS through their own resolver and fail on `script.google.com` (`ERR_NAME_NOT_RESOLVED`); measured on a real device, Safari succeeded 2 times out of 7 attempts *with* 3-way retry already in place — retry only masks it. Both pages start on `SHEET_URL_PROXY` and fall back to `SHEET_URL_DIRECT` via `markRouteFailed()` on the first network failure (that switch does not consume a retry), so a machine without the proxy still works. `index.html` fires a throwaway `verify` on load purely to settle the route **before** a teacher can submit — a POST is `no-cors`, so sending it down a dead route loses the teacher's data silently.
